...
gulp.task('build-run', 'Generate an unoptimized build and run it in the target environment.', (done) => {
    ...
    if (settings.BUILD_TARGET === 'chrome') command = `${command};chromium --user-data-dir=/tmp/ca11 --load-extension=${buildDir} --no-first-run`
...
gulp.task('scss-vendor', 'Generate vendor CSS.', () => {
    return helpers.scssEntry('./src/scss/ca11/vendor.scss', false)
})
