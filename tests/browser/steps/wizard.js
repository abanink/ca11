module.exports = function(_) {
    return {
        complete: async function(actor) {
            const {page} = actor
            // 1. Welcome:
            await _.screenshot(actor, 'wizard-welcome')
            await page.click('.t-btn-welcome-next')

            // 2. Media Permissions:
            await page.waitFor('.t-media-permission')
            await _.screenshot(actor, 'wizard-media-permission')
            ...
            await _.screenshot(actor, 'wizard-devices')
            ...
            await _.screenshot(actor, 'wizard-telemetry')
