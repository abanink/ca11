...
_.test('[bg] starting up sequence', function(t) {
    ...
    ca11.on('factory-defaults', () => {
        ...
        t.equal(
            ca11.store.get('schema'),
            ca11.store.schema, `storage: schema version (${ca11.store.schema}) present after factory reset`,
        )
    })
    // There is no schema in the database on a fresh start.
    t.equal(ca11.store.get('schema'), null, 'storage: schema absent on startup')
})
